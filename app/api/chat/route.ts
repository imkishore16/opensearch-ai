import { BraveResults } from '@/app/types';
...
export const POST = async (request: Request): Promise<Response> => {
  const body = (await request.json()) as { data: BraveResults; input: string };
  ...
  const messages: CoreMessage[] = [
    {
      role: 'system',
      content: `You are a search assistant that answers the user query based on search results. We already know this about the user, try to tell the user about this showing up!: ${memString}.
      Give answers in markdown format. the search results are ${body.data.web.results
        .map(
          (result) =>
            `${result.title}\n\n${result.description}\n\n${result.url}\n\n`
        )
